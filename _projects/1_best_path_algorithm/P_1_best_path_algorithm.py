...
def heuristica(posicion_actual, posicion_destino):
    costo_recto = 10
    costo_diagonal = 14

    (fila_actual, columna_actual) = posicion_actual
    (fila_destino, columna_destino) = posicion_destino

    # Distancia en cada eje
    d_filas = abs(fila_actual - fila_destino)
    d_columnas = abs(columna_actual - columna_destino)

    pasos_diagonales    = min(d_filas, d_columnas)
    pasos_rectos        = max(d_filas, d_columnas) - pasos_diagonales
